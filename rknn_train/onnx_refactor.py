...
def add_model_outputs(model: ModelProto, output_names: list[str], node_output_names: list[str]):
    """
    Добавляет выходы модели (graph.output), ссылающиеся на выходы существующих нод.

    :param model: ONNX модель (onnx.ModelProto), модифицируется inplace
    :param output_names: названия новых выходов модели (для ValueInfoProto)
    :param node_output_names: тензоры, на которые указывают новые выходы модели
    """
    ...
    for tensor_name in node_output_names:
        ...
        if value_info is None:
            # Если описание отсутствует — создадим без shape, с float типом по умолчанию
            new_output = helper.make_empty_tensor_value_info(
                name=tensor_name,
